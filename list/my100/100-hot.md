...
Summary: Use two pointers to traverse both lists and find the first shared node. Need to review.
...
Summary: Create copied nodes first, then reconnect next and random pointers using a map. Need to review.
...
Summary: Sort the linked list with merge sort by splitting and merging recursively. Need to review.
...
Summary: Implement LRU with a doubly linked list and hash map for O(1) get/put. Need to review.
...
class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        nums1, nums2 = [-inf] + nums1 + [inf], [-inf] + nums2 + [inf]
        ...
        while l <= r:
            m1 = (l + r) // 2
            m2 = total // 2 - m1 - 2
            leftA, rightA = A[m1], A[m1 + 1]
            leftB, rightB = B[m2], B[m2 + 1]
